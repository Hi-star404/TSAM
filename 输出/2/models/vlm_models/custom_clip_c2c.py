...
from models.vlm_models.global_compositional_spatial_expert import (
    GlobalCompositionalSpatialExpert,
)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, train_dataset, clip_model):
        ...
        self.use_mmn_spatial_refiner = getattr(
            cfg, 'use_mmn_spatial_refiner', False
        )
        self.mmn_temporal_conditioning = bool(getattr(
            cfg, 'mmn_temporal_conditioning', True
        ))
        self.mmn_temporal_relation_rank = int(getattr(
            cfg, 'mmn_temporal_relation_rank', 8
        ))
        # Unified global spatial expert: Stage-V (GSE) + Stage-C differential
        # relation experts. Temporal code low-rank-calibrates verb A_v(t) only.
        # Reuse the already-parsed kernel_sizes tuple above (do not re-read the
        # raw YAML string here ¡ª that would break TemporalExpertConv1d).
        self.c2c_global_spatial_expert = None
        if self.use_gse or self.use_mmn_spatial_refiner:
            temporal_gate_dim = len(kernel_sizes) if kernel_sizes else 4
            self.c2c_global_spatial_expert = GlobalCompositionalSpatialExpert(
                dim=cfg.emb_dim,
                num_object_tokens=len(train_dataset.objs),
                num_verb_tokens=len(train_dataset.attrs),
                temporal_input_dim=cfg.feat_dim,
                temporal_gate_dim=temporal_gate_dim,
                use_visual_stage=self.use_gse,
                use_composition_stage=self.use_mmn_spatial_refiner,
                gse_heads=getattr(cfg, 'gse_heads', 4),
                gse_dropout=getattr(cfg, 'gse_dropout', 0.1),
                gse_alpha_init=getattr(cfg, 'gse_alpha_init', 0.05),
                gse_beta_init=getattr(cfg, 'gse_beta_init', 0.05),
                mmn_bottleneck_dim=getattr(
                    cfg, 'mmn_spatial_bottleneck_dim', 128
                ),
                mmn_dropout=getattr(cfg, 'mmn_spatial_dropout', 0.15),
                mmn_residual_init=getattr(
                    cfg, 'mmn_spatial_residual_init', 0.05
                ),
                mmn_use_local_expert=False,
                mmn_visual_conditioning=False,
                mmn_temporal_conditioning=self.mmn_temporal_conditioning,
                mmn_context_residual_mod=False,
                mmn_temporal_relation_rank=self.mmn_temporal_relation_rank,
            )
            # Backward-compatible aliases used by older analysis scripts.
            self.c2c_gse_cond = (
                self.c2c_global_spatial_expert.visual_stage
            )
            self.c2c_mmn_spatial_refiner = (
                self.c2c_global_spatial_expert.composition_stage
            )
    ...
    def forward(
        ...
        if self.c2c_global_spatial_expert is not None:
            self.c2c_global_spatial_expert.clear_context()
            self.c2c_global_spatial_expert.set_temporal_context(
                temporal_video_embedding,
                temporal_gate_weights,
            )
            if self.use_gse:
                v_feat_c_t, o_feat_c = (
                    self.c2c_global_spatial_expert.refine_visual(
                        v_feat_c_t, o_feat_c
                    )
                )
    ...
    def condition_module(
        ...
        if (
                self.use_mmn_spatial_refiner
                and apply_mmn
                and self.c2c_global_spatial_expert is not None
        ):
            f_v_e_o_norm, f_o_e_v_norm = (
                self.c2c_global_spatial_expert.refine_composition(
                    f_v_e_o_norm,
                    f_o_e_v_norm,
                )
            )
