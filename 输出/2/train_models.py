...
def c2c_enhance(
    ...
    for epoch_index in range(config.epoch_start, config.epochs):
        ...
        log_training.write(
            ...
            "relation shaper {} | unified global expert {} | "
            "stage-c differential relation | "
            "verb temporal relation rank {} | "
            "mmn temporal conditioning {}\n"
            .format(
                epoch_index + 1,
                ...
                (core_model.c2c_global_spatial_expert is not None),
                int(getattr(
                    core_model,
                    'mmn_temporal_relation_rank',
                    0,
                )),
                bool(getattr(
                    core_model,
                    'mmn_temporal_conditioning',
                    False,
                )),
            )
        )
