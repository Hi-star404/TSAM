...
class CustomCLIP(nn.Module):
    ...
    def _natural_relation_train_scores(self, natural_pair_scores):
        """Pair CE targets over train pairs from the shared natural scores."""
        return natural_pair_scores[
            :,
            self.natural_train_pair_locations,
        ]
    ...
    def forward(
        ...
        if temporal_video_embedding is not None:
            ...
            if self.training:
                # Same temporal representation / natural text bank used for
                # component CE also provides the train-time relation CE.
                temporal_shaper_scores = (
                    self._natural_relation_train_scores(natural_pair_scores)
                )
