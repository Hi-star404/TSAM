...
class NaturalPairComponentFeedbackFusion(nn.Module):
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # Score fusion is optional and off by default: the temporal
            # branch is primarily a train-time relation regularizer.
            apply_fusion = (
                (self.training and self.fuse_during_training)
                or ((not self.training) and self.fuse_at_inference)
            )
            corrected_scores = (
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, train_dataset, clip_model):
        ...
        # Independent second temporal shaper is retired. Relation CE now
        # uses the same natural-pair temporal representation.
        ...
        if self.use_temporal_composition_branch:
            ...
            if self.temporal_relation_shaper_enabled:
                raise ValueError(
                    "temporal_relation_shaper_enabled is retired in the "
                    "unified temporal-branch design. Keep it false and use "
                    "temporal_shaper_loss_weight on the shared natural-pair "
                    "scores instead."
                )
            natural_prompts = []
            for attr, obj in train_dataset.pairs:
