...
TEMPORAL_DIAGNOSTIC_KEYS = [
    ...
    "temporal_acceptance_mean",
    "temporal_acceptance_std",
    "temporal_acceptance_open",
    "temporal_acceptance_closed",
]
...
def unpack_c2c_training_outputs(outputs):
    ...
    if len(outputs) == 18:
        # Legacy residual-boost packing without acceptance.
        return (outputs[:9],) + tuple(outputs[9:]) + (None,)
    if len(outputs) == 19:
        return (outputs[:9],) + tuple(outputs[9:])
    raise RuntimeError(
        "Expected 9 base, 18 legacy temporal, or 19 acceptance "
        "outputs, got %d." % len(outputs)
    )


def temporal_acceptance_supervision(
        global_scores,
        proposal_scores,
        acceptance_values,
        target_a,
        ...
        cosine_scale,
        target_temperature):
    """Teach the gate which detached proposal has lower per-sample risk."""
    global_logits = global_scores.detach().float() * float(cosine_scale)
    proposal_logits = proposal_scores.detach().float() * float(cosine_scale)

    def target_for(labels):
        global_risk = F.cross_entropy(global_logits, labels, reduction='none')
        proposal_risk = F.cross_entropy(
            proposal_logits, labels, reduction='none'
        )
        return torch.sigmoid(
            (global_risk - proposal_risk)
            / max(float(target_temperature), 1.0e-4)
        )

    with torch.no_grad():
        soft_target = target_for(target_a)
        if target_b is not None:
            soft_target = (
                float(target_mix) * soft_target
                + (1.0 - float(target_mix)) * target_for(target_b)
            )
    if acceptance_values.ndim == 2:
        acceptance = acceptance_values[:, 0]
    else:
        acceptance = acceptance_values
    acceptance = acceptance.float().clamp(1.0e-5, 1.0 - 1.0e-5)
    acceptance_logits = torch.logit(acceptance)
    loss = F.binary_cross_entropy_with_logits(
        acceptance_logits,
        soft_target.float(),
    )
    return loss, soft_target
...
def hard_mined_relation_ce(
        shaper_scores,
        target_indices,
        global_train_scores,
        hard_weight,
        easy_weight):
    """Weight relation CE toward samples where global top-1 is wrong."""
    ...
    global_wrong = (
        global_train_scores.detach().argmax(dim=1) != target_indices
    )
    weights = torch.where(
        global_wrong,
        per_sample.new_full((), float(hard_weight)),
        per_sample.new_full((), float(easy_weight)),
    )
    weight_sum = weights.sum().clamp_min(1.0e-6)
    loss = (per_sample * weights).sum() / weight_sum
    hard_fraction = global_wrong.float().mean()
    return loss, hard_fraction
...
def evaluate(model, dataset, config, fusion_alphas=None):
    ...
    # Inference uses the model's fixed (verb, object) fusion strengths when
    # full or selective fuse-at-inference is enabled; otherwise report zeros.
    ...
    fuse_at_inference = bool(getattr(
        core_model,
        'natural_pair_fuse_at_inference',
        False,
    ))
    selective_fuse = bool(getattr(
        core_model,
        'natural_pair_selective_fuse_at_inference',
        False,
    ))
    acceptance_on = bool(getattr(
        core_model,
        'natural_pair_acceptance_enabled',
        False,
    ))
    if not fuse_at_inference and not selective_fuse and not acceptance_on:
        alpha_verb, alpha_object = 0.0, 0.0
    elif fusion_alphas is not None:
...
def c2c_enhance(
    ...
    temporal_correction_weight = float(getattr(
        config,
        'temporal_correction_loss_weight',
        0.0,
    ))
    temporal_acceptance_weight = float(getattr(
        config,
        'temporal_acceptance_loss_weight',
        0.0,
    ))
    temporal_acceptance_temperature = float(getattr(
        config,
        'temporal_acceptance_target_temperature',
        1.0,
    ))
    ...
    if min(
            ...
            temporal_correction_weight,
            temporal_acceptance_weight) < 0.0:
        raise ValueError("Temporal loss weights must be non-negative.")
    if temporal_acceptance_temperature <= 0.0:
        raise ValueError(
            "temporal_acceptance_target_temperature must be positive."
        )
    ...
    for epoch_index in range(config.epoch_start, config.epochs):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                loss_temporal_correction = zero
                loss_temporal_acceptance = zero
                temporal_acceptance_target_mean = zero
                temporal_acceptance_mean = zero
                temporal_acceptance_open = zero
                ...
                relation_top1_accuracy = zero
                relation_hard_fraction = zero
                if temporal_corrected_scores is not None:
                    ...
                    if natural_feedback_weight > 0.0:
                        # Residual boosting: CE on the deployment-style fused
                        # proposal (detached global + fixed-alpha evidence).
                        # Gradients reach only the evidence head, training it
                        # to correct global's errors without disturbing
                        # samples global already gets right.
                    ...
                    if temporal_correction_weight > 0.0:
                        # Soft-accepted scores: same form as Old-1 correction.
                        loss_temporal_correction = (
                            natural_component_fusion_loss(
                                temporal_corrected_scores,
                                train_v_inds,
                                train_o_inds,
                                target_pair_a,
                    ...
                    if (
                            temporal_acceptance_weight > 0.0
                            and temporal_acceptance is not None
                    ):
                        global_train_for_accept = global_pair_scores[
                            :, train_v_inds, train_o_inds
                        ]
                        proposal_train_for_accept = temporal_proposal_scores[
                            :, train_v_inds, train_o_inds
                        ]
                        (
                            loss_temporal_acceptance,
                            acceptance_target,
                        ) = temporal_acceptance_supervision(
                            global_train_for_accept,
                            proposal_train_for_accept,
                            temporal_acceptance,
                            target_pair_a,
                            target_pair_b,
                            mix,
                            config.cosine_scale,
                            temporal_acceptance_temperature,
                        )
                        temporal_acceptance_target_mean = (
                            acceptance_target.float().mean()
                        )
                        temporal_acceptance_mean = (
                            temporal_acceptance.float().mean()
                        )
                        temporal_acceptance_open = (
                            temporal_acceptance.float().ge(0.5).float().mean()
                        )
                    (
                    ...
                    if (
                            ...
                            and temporal_shaper_scores is not None
                    ):
                        shaper_train_scores = (
                            temporal_shaper_scores.float()
                            * config.cosine_scale
                        )
                        global_train_for_hard = global_pair_scores[
                            :, train_v_inds, train_o_inds
                        ].detach()
                        hard_mining = bool(getattr(
                            core_model,
                            'temporal_shaper_hard_mining_enabled',
                            False,
                        ))
                        if hard_mining:
                            hard_w = float(getattr(
                                core_model,
                                'temporal_shaper_hard_weight',
                                1.0,
                            ))
                            easy_w = float(getattr(
                                core_model,
                                'temporal_shaper_easy_weight',
                                0.05,
                            ))
                            loss_temporal_shaper, hard_frac_a = (
                                hard_mined_relation_ce(
                                    shaper_train_scores,
                                    target_pair_a,
                                    global_train_for_hard,
                                    hard_w,
                                    easy_w,
                                )
                            )
                            if target_pair_b is not None:
                                loss_b, hard_frac_b = hard_mined_relation_ce(
                                    shaper_train_scores,
                                    target_pair_b,
                                    global_train_for_hard,
                                    hard_w,
                                ...
                                relation_hard_fraction = hard_frac_a
                        else:
                        ...
                        relation_top1_a = shaper_train_scores.argmax(
                            dim=1,
                        ).eq(target_pair_a).float().mean()
                        if target_pair_b is None:
                            relation_top1_accuracy = relation_top1_a
                        else:
                            relation_top1_b = shaper_train_scores.argmax(
                                dim=1,
                            ).eq(target_pair_b).float().mean()
                            relation_top1_accuracy = (
                                mix * relation_top1_a
                                + (1.0 - mix) * relation_top1_b
                            )
                    ...
                    # Match the deployed inference rule for the primary
                    # rescue/damage diagnostic when possible.
                    acceptance_on = bool(getattr(
                        core_model,
                        'natural_pair_acceptance_enabled',
                        False,
                    ...
                    if acceptance_on:
                        diagnostic_train_scores = corrected_train_scores
                    elif bool(getattr(
                            core_model,
                        ...
                        threshold = float(getattr(
                            core_model,
                            'natural_pair_selective_margin_threshold',
                            0.005,
                        ))
                        global_top1 = global_train_scores.argmax(dim=1)
                        proposal_topk = torch.topk(
                            proposal_train_scores.float(),
                            k=2,
                            dim=1,
                        )
                        proposal_top1 = proposal_topk.indices[:, 0]
                        proposal_margin = (
                            proposal_topk.values[:, 0]
                            - proposal_topk.values[:, 1]
                        )
                        select_mask = (
                            global_top1.ne(proposal_top1)
                            & (proposal_margin >= threshold)
                        )
                        diagnostic_train_scores = torch.where(
                            select_mask.unsqueeze(1),
                        ...
                        diagnostic_train_scores = proposal_train_scores
                    else:
                        diagnostic_train_scores = corrected_train_scores
                    # Counterfactual full proposal always reported separately.
                ...
                loss_temporal = (
                    ...
                    + temporal_correction_weight * loss_temporal_correction
                    + temporal_acceptance_weight * loss_temporal_acceptance
                )
                ...
                ensure_finite_training_loss(
                    unscaled_loss,
                    {
                        ...
                        "temporal_correction": loss_temporal_correction,
                        "temporal_acceptance": loss_temporal_acceptance,
                    },
                    epoch_index + 1,
                    batch_index + 1,
                )
            ...
            scalar_stats["temporal_correction"].append(
                loss_temporal_correction.detach().float().item()
            )
            scalar_stats["temporal_acceptance"].append(
                loss_temporal_acceptance.detach().float().item()
            )
            scalar_stats["temporal_acceptance_target"].append(
                temporal_acceptance_target_mean.detach().float().item()
            )
            scalar_stats["temporal_acceptance_mean"].append(
                temporal_acceptance_mean.detach().float().item()
            )
            scalar_stats["temporal_acceptance_open"].append(
                temporal_acceptance_open.detach().float().item()
            )
            ...
            scalar_stats["relation_top1"].append(
                relation_top1_accuracy.detach().float().item()
            )
            scalar_stats["relation_hard_fraction"].append(
                relation_hard_fraction.detach().float().item()
            )
        ...
        log_training.write(
            "epoch {} temporal correction/acceptance loss "
            "{:.6f}/{:.6f} | acceptance mean/target/open "
            "{:.6f}/{:.6f}/{:.6f}\n".format(
                epoch_index + 1,
                stat_mean("temporal_correction"),
                stat_mean("temporal_acceptance"),
                stat_mean("temporal_acceptance_mean"),
                stat_mean("temporal_acceptance_target"),
                stat_mean("temporal_acceptance_open"),
            )
        )
        log_training.write(
        ...
        infer_without_fusion = (
            (not core_model.natural_pair_fuse_at_inference)
            and (not selective_fuse)
            and (not acceptance_on)
        )
        if acceptance_on:
            fusion_diag_label = "soft acceptance"
        elif selective_fuse:
            fusion_diag_label = "selective fusion"
        elif infer_without_fusion:
            fusion_diag_label = "counterfactual fusion"
        else:
            fusion_diag_label = "deployed fusion"
        log_training.write(
        ...
        log_training.write(
            "epoch {} relation train top1 {:.6f} | hard fraction {:.6f}\n"
            .format(
                epoch_index + 1,
                stat_mean("relation_top1"),
                stat_mean("relation_hard_fraction"),
            )
        )
        log_training.write(
            "epoch {} temporal proposal rescue/damage/net "
            "(counterfactual) {:.6f}/{:.6f}/{:.6f}\n".format(
                epoch_index + 1,
        ...
        if gate_sample_count > 0:
            ...
            if gate_prior is not None:
                prior = (
                    gate_prior / gate_prior.sum().clamp_min(1.0e-8)
                ).cpu()
                gate_deviation = (gate_mean - prior).abs()
                log_training.write(
                    "epoch {} temporal gate prior deviation "
                    "k3/k5/k7/k9 {:.6f}/{:.6f}/{:.6f}/{:.6f}\n".format(
                        epoch_index + 1,
                        gate_deviation[0].item(),
                        gate_deviation[1].item(),
                        gate_deviation[2].item(),
                        gate_deviation[3].item(),
                    )
                )

        temporal_expert = core_model.c2c_temporal_composition_expert
        gate_residual_scale = temporal_expert.gate_residual_scale
        if gate_residual_scale is not None:
            gate_mode_message = (
                "bounded prior residual {:.4f}".format(
                    gate_residual_scale,
                )
            )
        else:
            gate_mode_message = (
                "adaptive temperature {:.4f}".format(
                    temporal_expert.gate_temperature,
                )
            )
        log_training.write(
            "epoch {} temporal branch settings {} | gate_norm {} | "
            "gate prior {} | role-robust natural joint text | "
            "train-without-fusion {} | infer-without-fusion {} | "
            "selective fuse {} (margin {:.4f}) | soft acceptance {} | "
            "relation shaper {} | hard mining {} | "
            "configured verb/object strengths {:.3f}/{:.3f} | "
            "prompt weights {} | decomposition iterations {} | "
            "component/feedback/route/relation loss weights "
            "{:.4f}/{:.4f}/{:.4f}/{:.4f} | "
            "correction/acceptance loss weights {:.4f}/{:.4f} | "
            "verb/object component mix {:.3f}/{:.3f} | "
            "component logit scale {:.4f} | "
            "feature residual {:.4f} | component/relation backbone grad "
            "{:.4f}/{:.4f} | unified natural relation CE {}\n"
            .format(
                epoch_index + 1,
                gate_mode_message,
                bool(getattr(temporal_expert.gate_norm, 'normalized_shape', None)),
                "/".join(
                    "{:.2f}".format(value)
                    for value in temporal_expert.gate_prior.detach().cpu().tolist()
                ) if temporal_expert.gate_prior is not None else "none",
                ...
                acceptance_on,
                bool(core_model.temporal_relation_shaper_enabled),
                bool(getattr(
                    core_model,
                    'temporal_shaper_hard_mining_enabled',
                    False,
                ...
                core_model.temporal_relation_backbone_gradient_scale,
                (
                    temporal_shaper_weight > 0.0
                    and (not core_model.temporal_relation_shaper_enabled)
                ),
            )
        )
