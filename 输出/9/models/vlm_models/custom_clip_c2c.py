...
class NaturalPairComponentFeedbackFusion(nn.Module):
    ...
    def __init__(
            ...
            fuse_at_inference=False,
            selective_fuse_at_inference=False,
            selective_margin_threshold=0.005,
            acceptance_enabled=False,
            acceptance_init=0.20,
        ...
        if float(selective_margin_threshold) < 0.0:
            raise ValueError(
                "selective_margin_threshold must be non-negative."
            )
        if acceptance_enabled and selective_fuse_at_inference:
            raise ValueError(
                "Soft acceptance and selective hard fusion cannot both "
                "be enabled."
            )
        ...
        self.fuse_at_inference = bool(fuse_at_inference)
        self.selective_fuse_at_inference = bool(selective_fuse_at_inference)
        self.selective_margin_threshold = float(selective_margin_threshold)
        self.acceptance_enabled = bool(acceptance_enabled)
        ...
        self.acceptance = (
            ComponentAwareNaturalAcceptance(
                initial_acceptance=float(acceptance_init),
            )
            if self.acceptance_enabled else None
        )
    ...
    def compute_natural_relation_features(
            self,
            temporal_video_embedding,
            natural_pair_text_features):
        """Natural scores + component feedback for relation CE only."""
        ...
        return natural_scores, component_feedback_scores
    ...
    def forward(
            ...
            verb_logits=None,
            object_logits=None,
            global_pair_o_scores=None,
            global_pair_v_scores=None,
            verb_condition_scores=None,
            object_condition_scores=None):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # Soft acceptance (Old-1 style) overrides hard fuse switches:
            # corrected = global + a * evidence, so late closing of a
            # recovers the global floor when fusion would damage.
            acceptance = None
            select_mask = None
            if self.acceptance_enabled:
                if (
                        verb_logits is None
                        or object_logits is None
                        or global_pair_o_scores is None
                        or global_pair_v_scores is None
                        or verb_condition_scores is None
                        or object_condition_scores is None
                ):
                    raise RuntimeError(
                        "Soft acceptance requires verb/object/condition "
                        "scores."
                    )
                proposal_valid = self._valid_pair_scores(proposal_scores)
                acceptance = self.acceptance(
                    ...
                    verb_logits.float(),
                    object_logits.float(),
                    self._valid_pair_scores(global_pair_o_scores.float()),
                    self._valid_pair_scores(global_pair_v_scores.float()),
                    self._valid_pair_scores(verb_condition_scores.float()),
                    self._valid_pair_scores(object_condition_scores.float()),
                )
                corrected_scores = (
                    global_reference
                    + acceptance.view(-1, 1, 1) * proposal_evidence_full
                )
                apply_fusion = True
            else:
                # Train: optional full fusion. Infer: full / selective / none.
                if self.training:
                    apply_fusion = self.fuse_during_training
                elif self.selective_fuse_at_inference:
                    proposal_valid = self._valid_pair_scores(proposal_scores)
                    global_top1 = global_valid.argmax(dim=1)
                    proposal_topk = torch.topk(proposal_valid, k=2, dim=1)
                    proposal_top1 = proposal_topk.indices[:, 0]
                    proposal_margin = (
                        proposal_topk.values[:, 0] - proposal_topk.values[:, 1]
                    )
                    select_mask = (
                        global_top1.ne(proposal_top1)
                        & (
                            proposal_margin
                            >= self.selective_margin_threshold
                        )
                    )
                    apply_fusion = True
                else:
                    apply_fusion = self.fuse_at_inference

                if not apply_fusion:
                    corrected_scores = global_reference
                elif select_mask is None:
                    corrected_scores = proposal_scores
                else:
                    corrected_scores = torch.where(
                        select_mask.view(-1, 1, 1),
                        proposal_scores,
                        global_reference,
                    )
            ...
            if acceptance is not None:
                fusion_weight = acceptance
            elif select_mask is not None:
                applied_strength = (
                    self.verb_feedback_strength
                    + self.object_feedback_strength
                )
                fusion_weight = select_mask.to(
                    dtype=natural_evidence.dtype
                ) * applied_strength
            else:
                applied_strength = (
                    self.verb_feedback_strength
        ...
        return (
            ...
            natural_scores.to(dtype=source_dtype),
            None if acceptance is None else acceptance,
        )
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, train_dataset, clip_model):
        ...
        relation_backbone_gradient_scale = getattr(
            cfg, 'temporal_relation_backbone_gradient_scale', None
        )
        if isinstance(relation_backbone_gradient_scale, str):
            stripped = relation_backbone_gradient_scale.strip().lower()
            relation_backbone_gradient_scale = (
                None if stripped in ('', 'none', 'null')
                else float(stripped)
            )
        if relation_backbone_gradient_scale is None:
            relation_backbone_gradient_scale = (
                self.temporal_composition_backbone_gradient_scale
            )
        self.temporal_relation_backbone_gradient_scale = float(
            relation_backbone_gradient_scale
        )
        if not 0.0 <= self.temporal_relation_backbone_gradient_scale <= 1.0:
            raise ValueError(
                "temporal_relation_backbone_gradient_scale must lie "
                "in [0, 1]."
            )
        ...
        # Scheme-1 default: never fuse temporal scores at inference; the
        # temporal branch shapes the shared trunk during training only.
        self.natural_pair_fuse_at_inference = bool(getattr(
            cfg, 'natural_pair_fuse_at_inference', False
        ))
        self.natural_pair_selective_fuse_at_inference = bool(getattr(
            cfg, 'natural_pair_selective_fuse_at_inference', False
        ))
        self.natural_pair_selective_margin_threshold = float(getattr(
            cfg, 'natural_pair_selective_margin_threshold', 0.005
        ))
        # Soft acceptance gate (Old-1). When enabled, overrides hard fuse
        # switches with corrected = global + a * fixed-alpha evidence.
        self.natural_pair_acceptance_enabled = bool(getattr(
            cfg, 'natural_pair_acceptance_enabled', False
        ))
        self.temporal_acceptance_init = float(getattr(
            cfg, 'temporal_acceptance_init', 0.20
        ))
        # Independent "1"-style trunk shaper (train-only, never fused).
        self.temporal_relation_shaper_enabled = bool(getattr(
            cfg, 'temporal_relation_shaper_enabled', False
        ))
        self.temporal_shaper_hard_mining_enabled = bool(getattr(
            cfg, 'temporal_shaper_hard_mining_enabled', False
        ))
        self.temporal_shaper_hard_weight = float(getattr(
            cfg, 'temporal_shaper_hard_weight', 1.0
        ))
        self.temporal_shaper_easy_weight = float(getattr(
            cfg, 'temporal_shaper_easy_weight', 0.05
        ))
        if self.temporal_shaper_hard_weight < 0.0:
            raise ValueError("temporal_shaper_hard_weight must be >= 0.")
        if self.temporal_shaper_easy_weight < 0.0:
            raise ValueError("temporal_shaper_easy_weight must be >= 0.")
        if (
            self.natural_pair_selective_fuse_at_inference
            and self.natural_pair_fuse_at_inference
            and (not self.natural_pair_acceptance_enabled)
        ):
            raise ValueError(
                "Enable either selective fusion or full fuse_at_inference, "
                "not both."
            )
        if (
            self.natural_pair_acceptance_enabled
            and self.natural_pair_selective_fuse_at_inference
        ):
            raise ValueError(
                "Soft acceptance and selective hard fusion cannot both "
                "be enabled."
            ...
            with torch.random.fork_rng(devices=[]):
                ...
                self.c2c_natural_pair_probabilistic_fusion = (
                    NaturalPairComponentFeedbackFusion(
                        ...
                        fuse_at_inference=(
                            self.natural_pair_fuse_at_inference
                        ),
                        selective_fuse_at_inference=(
                            self.natural_pair_selective_fuse_at_inference
                        ),
                        selective_margin_threshold=(
                            self.natural_pair_selective_margin_threshold
                        ),
                        acceptance_enabled=(
                            self.natural_pair_acceptance_enabled
                        ),
                        acceptance_init=self.temporal_acceptance_init,
                ...
                # Map each train-pair target index onto the natural-pair
                # text bank so relation CE can share that representation.
                pair_lookup = {
                    (
                        int(pair_indices[index, 0]),
                        int(pair_indices[index, 1]),
                    ): index
                    for index in range(pair_indices.shape[0])
                }
                train_pair_locations = []
                for attr, obj in train_dataset.train_pairs:
                    ...
                    if key not in pair_lookup:
                        raise KeyError(
                            "Train pair %s is missing from the natural "
                            "pair bank." % ((attr, obj),)
                        )
                    train_pair_locations.append(pair_lookup[key])
                self.register_buffer(
                    'natural_train_pair_locations',
                    torch.tensor(train_pair_locations, dtype=torch.long),
                    persistent=False,
                )
            if self.temporal_relation_shaper_enabled:
                # Training-only trunk shaper (run "1" mechanism). Scores are
                # never fused at evaluation; only hard-mined relation CE
                # shapes the shared backbone.
                with torch.random.fork_rng(devices=[]):
    ...
    @staticmethod
    def _scaled_backbone_input(video_features, gradient_scale, training):
        if (not training) or gradient_scale <= 0.0:
            return video_features.detach()
    ...
    def _build_temporal_video_embedding(
            self,
            video_features,
            gradient_scale,
            return_gate=False):
        temporal_branch_input = self._scaled_backbone_input(
            video_features,
            gradient_scale,
            self.training,
        )
        ...
        temporal_video_embedding = temporal_refined_features.mean(dim=-1)
        if return_gate:
            return temporal_video_embedding, temporal_gate_weights
        return temporal_video_embedding

    def _natural_relation_train_scores(
            self,
            natural_pair_scores,
            natural_component_feedback_scores,
            obj_logits,
            global_pair_scores):
        """Calibrated train-pair relation CE on the shared natural scores.

        Mirrors the retired independent shaper: subtract object marginal,
        re-anchor with global object logits, row-standardize, then align
        each row to the detached global train-pair mean/std.
        """
        fusion = self.c2c_natural_pair_probabilistic_fusion
        train_locations = self.natural_train_pair_locations
        pair_indices = fusion.pair_indices
        train_pairs = pair_indices[train_locations]
        train_obj_idx = train_pairs[:, 1]

        pair_scores = natural_pair_scores[:, train_locations].float()
        object_effect = natural_component_feedback_scores[
            :,
            fusion.num_verbs:,
        ].float()
        relation_scores = pair_scores - object_effect[:, train_obj_idx]
        object_anchor = obj_logits[:, train_obj_idx].detach().float()

        proposal_shape = self._row_standardize(relation_scores)
        proposal_shape = proposal_shape + self._row_standardize(object_anchor)
    ...
    def forward(
        ...
        if self.use_temporal_composition_branch:
            ...
            temporal_video_embedding, temporal_gate_weights = (
                self._build_temporal_video_embedding(
                    video_features,
                    component_scale,
                    return_gate=True,
                )
        ...
        if temporal_video_embedding is not None:
            ...
            (
                ...
                global_pair_o_scores=p_pair_o,
                global_pair_v_scores=p_pair_v,
                verb_condition_scores=p_v_con_o,
                object_condition_scores=p_o_con_v,
            )
            if self.training:
                if self.temporal_relation_shaper_enabled:
                    # Prefer the independent full-gradient "1"-style shaper.
                    ...
                    relation_scale = (
                        self.temporal_relation_backbone_gradient_scale
                    )
                    if abs(relation_scale - component_scale) > 1.0e-6:
                        relation_video_embedding = (
                            self._build_temporal_video_embedding(
                                video_features,
                                relation_scale,
                            )
                        ...
                        (
                            relation_natural_scores,
                            relation_component_feedback_scores,
                        ) = (
                            self.c2c_natural_pair_probabilistic_fusion
                            .compute_natural_relation_features(
                                relation_video_embedding,
                                self.natural_pair_text_features,
                            )
                        )
                        temporal_shaper_scores = (
                            self._natural_relation_train_scores(
                                relation_natural_scores,
                                relation_component_feedback_scores,
                                obj_logits,
                                global_pair_scores,
