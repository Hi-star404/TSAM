...
import numpy as np
from .global_spatial_expert import GlobalSpatialExpert
...
class C2C(nn.Module):
    ...
    def __init__(self, dset, cfg):
        ...
        if self.use_gse:
            self.gse_main = GlobalSpatialExpert(
                dim=cfg.emb_dim,
            ...
            self.gse_cond = GlobalSpatialExpert(
                dim=cfg.emb_dim,
                num_heads=getattr(cfg, 'gse_heads', 4),
                dropout=getattr(cfg, 'gse_dropout', 0.1),
                alpha_init=getattr(cfg, 'gse_alpha_init', 0.05),
    ...
    def val_forward_closed(self, x, pairs, visual=False):
        ...
        v_feat_t = self.VE1(vid_feat)
        if self.use_gse:
            v_feat_t, o_feat = self.gse_main(v_feat_t, o_feat)
        ...
        v_feat_c_t = self.VE2(vid_feat)
        if self.use_gse:
            v_feat_c_t, o_feat_c = self.gse_cond(v_feat_c_t, o_feat_c)
