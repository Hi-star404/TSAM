            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                if temporal_corrected_scores is not None:
                    ...
                    if natural_feedback_weight > 0.0:
                        # Residual boosting on the deployment-style proposal
                        # (detach(global) + fixed-alpha evidence). During
                        # train-no-fuse, corrected_scores == global, so CE on
                        # corrected cannot reach the evidence head; proposal
                        # does, and trains it to fix global's errors.
