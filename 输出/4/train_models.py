...
def evaluate(model, dataset, config, fusion_alphas=None):
    ...
    # Inference uses the model's fixed (verb, object) fusion strengths only
    # when fuse_at_inference is enabled; otherwise report zeros (global-only).
...
def c2c_enhance(
    ...
    for epoch_index in range(config.epoch_start, config.epochs):
        ...
        fusion_diag_label = (
            ...
            if infer_without_fusion else "deployed fusion"
        )
        log_training.write(
